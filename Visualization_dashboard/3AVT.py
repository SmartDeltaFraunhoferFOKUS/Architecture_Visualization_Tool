...
import settings
import yaml
from core import the_settings_tab, about_tab, dashboard_tab, gsr_dashboard
...
def get_dashboard_main(db_obj:db_actions.db_adm, user_config:settings.user_configs):
    ...
    * user_config: user configurations       
    ...
    dashboard, GSR_dash, settings, about = st.tabs([":bar_chart: AVT Dashboard", ":bar_chart: GSR Dashboard",  ":gear: Settings", ":information_source: About"])

    #GSR Dashboard Tab
    with GSR_dash: 
        #define the dashboard container in the form of an expander (this give a nice boarder)
        with st.expander("", expanded=True):  
            '''### GSR Visualization Dashboard'''
            st.divider()
            gsr_dashboard.draw_main_dashboard(db_obj)
    #AVT Dashboard Tab
    with dashboard: 
        #define the dashboard container in the form of an expander (this give a nice boarder)
        with st.expander("", expanded=True):  
